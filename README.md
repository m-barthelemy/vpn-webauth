...
All the configuration parameters have to passed as environment variables.
  ...
  - `VPNWA_SSLMODE`: whether and how SSL is enabled. Default: `off`. Can be `auto`, `custom`, `proxy`, `off`.
    ...
    > NOTE: `auto` will force the application to listen on both ports 80 and 443, and will ignore the value of `VPNWA_PORT`. These ports are privileged, meaning that you will need to start the application as root using `sudo`, or executing `chmod u+s vpn-webauth` to grant the binary admin permissions.
    > `custom` will let you specify a custom certificate and key using `VPNWA_SSLCERTPATH` and `VPNWA_SSLKEYPATH`.
    > `proxy` delegates the responsibility of providing SSL termination to an external component or proxy. But unlike `off`, it sets the `Secure` flag for the cookies generated by the application.
  - `VPNWA_SSLCERTPATH`: path to the SSL certificate. Optional. Default: `/ssl/key.pem`.
  - `VPNWA_SSLKEYPATH`: path to the SSL certificate private key. Optional. Default: `/ssl/cert.pem`.
